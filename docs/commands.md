...
| `TYPE` | `TYPE <key>` | Get the data type of a key | `string`, `list`, `set`, … |
...
| Command | Signature | Description | Returns |
|---------|-----------|-------------|---------|
| `S_SET` | `S_SET <key> <value> [ttl]` | Create a string key (errors if key exists — use S_UPSERT to overwrite) | `:1` |
| `S_UPSERT` | `S_UPSERT <key> <value> [ttl]` | Create or overwrite a string key unconditionally | `:1` |
| `S_GET` | `S_GET <key>` | Get the string value | String or `nil` |
| `S_INCR` | `S_INCR <key>` | Increment an integer string by 1 | `:1` (success indicator) |
| `S_GINCR` | `S_GINCR <key>` | Get the current value, then increment by 1 | Old integer value |
| `S_INCRBY` | `S_INCRBY <key> <n>` | Increment by `n` | `:1` (success indicator) |
| `S_GINCRBY` | `S_GINCRBY <key> <n>` | Get the current value, then increment by `n` | Old integer value |
| `S_APPEND` | `S_APPEND <key> <value>` | Append `value` to the existing string | `:1` |
| `S_RPAD` | `S_RPAD <key> <len> <char>` | Right-pad the string to `len` with `char` | `:1` |
| `S_LPAD` | `S_LPAD <key> <len> <char>` | Left-pad the string to `len` with `char` | `:1` |
...
| `L_MPOP` | `L_MPOP <key> <n>` | Remove and return N elements from the tail | Array or `nil` |
| `L_MDEQUEUE` | `L_MDEQUEUE <key> <n>` | Remove and return N elements from the head | Array or `nil` |
| `L_TRIMR` | `L_TRIMR <key> <n>` | Keep only the first `n` elements (trim from the right) | `:1` |
| `L_TRIML` | `L_TRIML <key> <n>` | Keep only the last `n` elements (trim from the left) | `:1` |
| `L_MOVE` | `L_MOVE <key1> <key2>` | Append `key2`'s elements onto `key1`'s tail; `key2` is deleted. Surviving key is `key1`. | `:1` |

---

## Set Commands

All set commands are prefixed with `SET_`. The key must hold a set value; using a set command on a key of a different type returns a `WRONGTYPE` error.

| Command | Signature | Description | Returns |
|---------|-----------|-------------|---------|
| `SET_ADD` | `SET_ADD <key> <value>` | Add a value to the set; creates the set if it does not exist | `:1` |
| `SET_GET` | `SET_GET <key> [n]` | Return all elements, or N random elements | Array or `nil` |
| `SET_DEL` | `SET_DEL <key> <value>` | Remove a specific element from the set | `:1` or `:0` |
| `SET_GETDEL` | `SET_GETDEL <key> <value>` | Remove and return a specific element | Element or `nil` |
| `SET_POP` | `SET_POP <key> [n]` | Remove and return N random elements (default 1) | Array or `nil` |
| `SET_LEN` | `SET_LEN <key>` | Return the cardinality of the set | Integer or `nil` |
...
S_INCR counter            # → 1
S_GET counter             # → "1"
S_UPSERT counter 100      # overwrite unconditionally

# List basics
L_PREPEND queue task1
L_APPEND  queue task2
L_DEQUEUE queue           # → "task1"
L_MPOP queue 2            # → pop 2 elements from tail

# Set basics
SET_ADD tags inmemory
SET_ADD tags database
SET_ADD tags julia
SET_LEN tags              # → 3
SET_GET tags              # → ["inmemory", "database", "julia"] (unordered)
SET_POP tags 1            # → removes and returns 1 random element
